...
import { ArrowRight, ShoppingBag, Zap, ShieldCheck, Star, Sparkles } from "lucide-react";
    ...
    >
      ...
      <div className="container mx-auto px-4 z-10 flex flex-col lg:flex-row items-center gap-12 lg:gap-20 grow pb-12">
        ...
        <div className="flex-1 text-center lg:text-left order-2 lg:order-1">
          ...
          >
            Curating the finest electronics and lifestyle pieces for the digital connoisseur. 
            absolute performance meets sculptural aesthetics.
          </Motion.p>
          ...
          >
            <Link to="/products">
              ...
              >
                <span className="relative z-10 flex items-center gap-3">
                  Shop Catalog <ArrowRight size={18} className="group-hover:translate-x-2 transition-transform" />
                </span>
              </button>
            </Link>
            ...
            <div className="flex -space-x-3">
              ...
              <div className="pl-6 flex flex-col justify-center">
                <span className="text-[10px] font-black uppercase tracking-widest leading-none mb-1">50k+ Members</span>
        ...
        <div className="flex-1 w-full order-1 lg:order-2">
          <AnimatePresence mode="wait">
            {loading ? (
              <div key="loader" className="aspect-square rounded-[60px] bg-secondary/20 animate-pulse" />
            ) : (
              <Motion.div
              ...
              >
                ...
                {/* Main Product Image */}
                <Link to={`/product/${activeProduct?._id}`} className="relative group select-none">
                  <img
                    src={activeProduct?.images?.[0] || "https://via.placeholder.com/800"}
                  ...
                  >
                    <p className="text-[8px] font-black uppercase tracking-widest text-primary mb-1">New Arrival</p>
                    <p className="font-bold text-sm tracking-tighter truncate max-w-[150px]">{activeProduct?.name}</p>
                    <p className="text-xl font-black text-foreground">₹{activeProduct?.price.toLocaleString()}</p>
                  </Motion.div>
                  ...
                  >
                    <div className="flex items-center gap-3">
                       ...
                       <div>
                          ...
                          <p className="font-bold text-xs">{activeProduct?.category}</p>
                       </div>
                    </div>
                  </Motion.div>
                </Link>
      ...
      <div className="bg-secondary/5 border-t border-border/50 py-12 z-10 backdrop-blur-sm">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            <FeatureItem icon={<Zap />} title="Hyper-Fast Delivery" desc="Next day shipping standard" />
            <FeatureItem icon={<ShieldCheck />} title="Encrypted Matrix" desc="100% Secure token payments" />
            <FeatureItem icon={<Star />} title="Artisan Curated" desc="Only the peak of technology" />
            <FeatureItem icon={<Sparkles />} title="Flash Updates" desc="New arrivals every 48 hours" />
          </div>
        </div>
      </div>
    </section>
...
const FeatureItem = ({ icon, title, desc }) => (
  <div className="flex items-center gap-5 group">
    ...
    <div className="flex flex-col">
      <h4 className="text-[10px] font-black uppercase tracking-[0.2em] mb-1">{title}</h4>
