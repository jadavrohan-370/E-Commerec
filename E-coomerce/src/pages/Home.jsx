...
const Home = () => {
  ...
  useEffect(() => {
    ...
    if (!loading && featuredProducts.length > 0) {
      gsap.fromTo(featuredRef.current.querySelectorAll('.featured-item'),
    ...
    <div className="flex flex-col w-full overflow-hidden bg-background">
      ...
      <section className="py-24" ref={categoriesRef}>
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
            ...
            >
              View All Collections <ArrowRight className="group-hover:translate-x-1 transition-transform" size={20} />
            </Link>
          </div>
      ...
      <section className="py-24 bg-secondary/30" ref={featuredRef}>
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            <span className="text-primary font-bold tracking-widest uppercase text-xs mb-4 block">New Season</span>
            <h2 className="text-4xl md:text-5xl font-bold tracking-tighter">Featured Arrivals</h2>
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
              {featuredProducts.map((product, idx) => (
                <ProductItem 
                  ...
                  img={product.images?.[0] || product.image || "https://via.placeholder.com/400"}
                  isNew={idx === 0}
                  isHot={idx === 1}
                />
              ))}
          ...
          <div className="relative h-[600px] rounded-4xl overflow-hidden group">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-8">
              ...
              <p className="text-white/80 text-lg md:text-xl max-w-2xl mb-10">
                Crafted for those who appreciate the intersection of high performance and sculptural aesthetics.
              </p>
      ...
      <section className="py-24 border-t">
        <div className="container mx-auto px-4 max-w-4xl text-center">
          <h3 className="text-3xl md:text-4xl font-bold mb-6 tracking-tight">Stay ahead of the curve</h3>
          <p className="text-muted-foreground text-lg mb-10">
            Join 50,000+ tech enthusiasts and get exclusive early access to our limited runs.
          </p>
...
const ProductItem = ({ id, name, price, category, img, isNew, isHot }) => (
  <div className="featured-item group flex flex-col">
    <Link to={`/product/${id}`} className="relative aspect-4/5 overflow-hidden rounded-2xl bg-secondary mb-4">
      <img 
        src={img} 
    ...
    <div className="flex flex-col gap-1">
      <span className="text-[10px] uppercase tracking-[0.2em] font-bold text-muted-foreground">{category}</span>
      <h3 className="font-bold text-lg leading-tight truncate">
        <Link to={`/product/${id}`} className="hover:opacity-70 transition-opacity">{name}</Link>
      </h3>
